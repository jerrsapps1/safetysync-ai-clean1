  ...
  app.post("/api/leads", async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        ...
        [name, email, company, role, message, demoRequest, heardFrom]
      );
...
<body>
  ...
  app.get('/api/lifers-claimed', async (req, res) => {
    try {
      const lifersClaimed = 127; // Replace with DB count
      res.json({ count: lifersClaimed });
    } catch (err) {
      console.error('Error fetching lifer count:', err);
      res.status(500).json({ error: 'Failed to fetch lifer count' });
    }
  });
