...
export default function WorkspacePage() {
  ...
  const [extendedLayouts, setExtendedLayouts] = useState(() => {
    ...
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        console.log('Loaded saved extended layouts:', parsed);
        return parsed;
      } catch (error) {
        console.error('Error parsing saved extended layouts:', error);
      }
    }
...
Mike,Johnson,EMP003,mike.johnson@company.com,Manufacturing,Supervisor,active`;
  ...
  const handleExtendedLayoutChange = useCallback((newLayout: any, allLayouts: any) => {
    if (JSON.stringify(allLayouts) !== JSON.stringify(extendedLayouts)) {
      console.log('Extended layout changed, updating layouts');
      setExtendedLayouts(allLayouts);
      localStorage.setItem('extended-dashboard-layouts', JSON.stringify(allLayouts));
    }
  }, [extendedLayouts]);
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {activeTab === "unified-dashboard" && (
            <div className="space-y-6">
              ...
              {/* Extended Widget Grid for Dashboard Content */}
              <div className="mt-8">
                <ResponsiveGridLayout
                  className="layout"
                  layouts={extendedLayouts}
                  onLayoutChange={handleExtendedLayoutChange}
                  breakpoints={{ lg: 1200, md: 996, sm: 768, xs: 480, xxs: 0 }}
                  cols={{ lg: 12, md: 10, sm: 6, xs: 4, xxs: 2 }}
                  rowHeight={60}
                  isDraggable={true}
